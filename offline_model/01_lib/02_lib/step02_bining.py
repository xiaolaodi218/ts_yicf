...
'''
binContVar函数可以对 连续型变量 进行最优分箱
reduceCats根据分段指标表将 字符型数据 分为指定分段数
'''
...
#             ix = temp_cont[temp_cont.index].index\.min()
...
def _getNewBins(sub, i):
    ...
    l = len(sub)
    total = sub['total'].sum()
    flag_0 = sub['flag_0'].sum()
    flag_1 = sub['flag_1'].sum()
    ...
    df = pd.DataFrame()
    df = df.append([i, lower, upper, flag_0, flag_1, total], ignore_index=True).T
    df.columns = ['bin', 'lower', 'upper', 'flag_0', 'flag_1', 'total']
    return df
...
def binContVar(x, y, method, mmax=5, Acc=0.01, target=1, adjust=0.0001):
   ...
   # _check_target_binary(y)
    ...
    bin_map = _EqualWidthBinMap(x, Acc, adjust=adjust)  #将数据切分为100份
    ...
    temp_cont = temp_cont.rename(columns={0:'flag_0'})
    temp_cont = temp_cont.rename(columns={1:'flag_1'})
