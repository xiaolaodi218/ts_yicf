...
TOTAL_TAT_b1=10+(_n_-1)*40;
TOTAL_TAT_e1=46+(_n_-1)*40;
...
from mtd_base(where=(复贷申请 in (&&复贷申请_&i) and 审核开始日^=&nt  and 银策略筛选 in (&&银策略筛选_&i)  and loc_abmoduleflag in (&&loc_abmoduleflag&i) and 订单类型 in (&&订单类型_&i))) group by 审核开始月份;
quit;
proc sql;
create table test1 as
select 放款月份 as 时间 format=$20.,count(*) as 放款笔数,sum(loan_amt) as 放款金额 from mtd_base(where=(放款月份 ^="&&放款月份_&i" and 复贷申请 in (&&复贷申请_&i) and 放款日^=&nt and 银策略筛选 in (&&银策略筛选_&i) and loc_abmoduleflag in (&&loc_abmoduleflag&i) and 订单类型 in (&&订单类型_&i))) group by 放款月份;
quit;
proc sql;
...
sum(case when 审批结果="人工复核中" then 1 else 0 end) as 人工复核中,
sum(case when 审批结果="人工取消" then 1 else 0 end) as 人工取消
from mtd_base(where=(复贷申请 in (&&复贷申请_&i) and &bt.<=审核开始日<=&dt. and 银策略筛选 in (&&银策略筛选_&i) and loc_abmoduleflag in (&&loc_abmoduleflag&i) and 订单类型 in (&&订单类型_&i))) group by 审核开始日期;
quit;
proc sql;
create table test4 as
select 放款日期 as 时间 format=$20.,count(*) as 放款笔数,sum(loan_amt) as 放款金额 
from mtd_base(where=(放款月份 ^="&&放款月份_&i" and 复贷申请 in (&&复贷申请_&i) and &bt.<=放款日<=&dt. and 银策略筛选 in (&&银策略筛选_&i) and loc_abmoduleflag in (&&loc_abmoduleflag&i) and 订单类型 in (&&订单类型_&i))) group by 放款日期;
quit;
proc sql;
...
filename DD DDE "EXCEL|[MTD_米粒审批日报.xlsx]Sheet1!r&&totalb1_row_&i..c1:r&&totale1_row_&i..c8";
data _null_;set test_combine;file DD;put 时间 系统审核中 系统拒绝 系统通过 人工拒绝 人工通过 人工复核中 人工取消 ;run;
filename DD DDE "EXCEL|[MTD_米粒审批日报.xlsx]Sheet1!r&&totalb1_row_&i..c13:r&&totale1_row_&i..c14";
