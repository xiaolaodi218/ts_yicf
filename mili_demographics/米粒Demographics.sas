...
select a.apply_code,a.申请提交月份,a.申请通过,a.申请拒绝,a.复贷申请,b.RESIDENCE_CITY_NAME,b.JOB_COMPANY_CITY_NAME,a.申请结果,a.申请提交日期,a.来源渠道,a.period,b.DEGREE_NAME,b.MARRIAGE_NAME,b.SEX_NAME,b.ID_CARD,b.RESIDENCE_PROVINCE_NAME,b.RESIDENCE_CONDITION_NAME,
b.JOB_COMPANY_CONDITION_NAME,b.CURR_JOB_SENIORITY_NAME,b.JOB_COMPANY_PROVINCE_NAME,b.MONTH_SALARY_NAME,c.审核处理月份,c.审核处理日期,c.refuse_name
...
网龄月份=intck("month",loc_register_date,申请提交日);
...
过去第1个月被叫区间 过去第1个月主叫区间 
过去第2个月被叫区间 过去第2个月主叫区间 
过去第3个月被叫区间 过去第3个月主叫区间
过去3个月被叫区间 过去3个月主叫区间 
...
company_province_g  salary_g loc_appsl_g loc_txlsl_g  申请提交点_g 七天多台 一月多台 三月多台 
芝麻分区间 通话距离申请区间 近三最静默区间 近三静默区间 近一最静默区间 近一静默区间 
app个数区间 app种类区间 手容占比 过一通次 过三通次 过一被叫比 过一短通次 过一短主叫 过三短通次 过三短主叫 过一紧通次 过三紧通次 过一户通次 过三户通次 
过一三线通次 过三三线通次 最一被催次 最三被催次 最一被特殊次  最三被特殊次  $20.;
...
else if 350<=loc_zmscore<500 then 芝麻分区间="6.[350,499]";
else if 500<=loc_zmscore<550 then 芝麻分区间="5.[500,549]";
else if 550<=loc_zmscore<600 then 芝麻分区间="4.[550,599]";
else if 600<=loc_zmscore<650 then 芝麻分区间="3.[600,649]";
else if 650<=loc_zmscore<700 then 芝麻分区间="2.[650,699]";
else if loc_zmscore>=700 then 芝麻分区间="1.[700+]";
...
if loc_addresscnt=0 then 收货地址区间="1.0个";
else if  0<loc_addresscnt<=4 then 收货地址区间="2.1-4个";
else if  loc_addresscnt>4 then 收货地址区间="3.4个+";
else if loc_addresscnt=. then 收货地址区间="4.无";
...
if 0<=loc_appsl<=5 then loc_appsl_g="1. 0-5个";
else if 5<loc_appsl<=10 then loc_appsl_g="2. 6-10个";
else if 10<loc_appsl<=15 then loc_appsl_g="3. 11-15个";
else if loc_appsl>15 then loc_appsl_g="4. 16个以上";
