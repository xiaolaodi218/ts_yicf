...
if len(sys.argv) > 1:
    ...
    start_dt = datetime.datetime(2016,12,1).replace(hour=0, minute=0, second=0, microsecond=0)

# end date as start_date + 1 day
end_dt = datetime.datetime(2017,6,5).replace(hour=0, minute=0, second=0, microsecond=0)
...
req_csv_file = tmp_dir + '\\'+'req_bqs_face2016_12.1-2017_06.5' + ".csv"
...
for (id, req_data) in query_cursor:
    ...
    if req_data_json['eventType'] != 'faceRecognition':
        continue
