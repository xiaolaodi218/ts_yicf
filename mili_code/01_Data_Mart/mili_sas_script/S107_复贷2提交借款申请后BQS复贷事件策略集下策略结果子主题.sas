...
***贷款事件;
data loan;
set submart.reloansimpleBQS_submart(where = (event_name = "custreloan") 
...
***基本规则策略;
data jbgz_strategy;
set dpRaw.bqs_strategy_result(where = (strategy_name = "复贷基本规则策略"));
...
select a.apply_code, a.execut状态, a.execut日期, a.execut月份, a.os_type, a.main_info_id, b.strategy_result_id, b.strategy_decision as 复贷基本规则策略结果
...
***复贷准入策略;
data fdzr_strategy;
set dpRaw.bqs_strategy_result(where = (strategy_name = "复贷准入策略"));
rename id = strategy_result_id;
drop date_created last_updated reject_value review_value risk_type strategy_id strategy_mode tips;
run;
proc sort data = fdzr_strategy nodupkey; by main_info_id descending strategy_result_id; run;
proc sort data = fdzr_strategy nodupkey; by main_info_id; run;

proc sql;
create table fdzr as
select a.main_info_id, b.strategy_result_id, b.strategy_decision as 复贷准入策略结果
from loanSet_result as a left join 
fdzr_strategy as b on a.main_info_id = b.main_info_id
;
quit;
...
proc sort data = fdzr nodupkey; by main_info_id; run;


data submart.reloansimpleBQS_loan_submart;
merge jbgz(in = a) jxl(in = b) llkz(in = c) eysq(in = d) ycsq(in = e) fsyys(in = f) fsds(in = g) glgz(in = h) fdzr(in = i);
by main_info_id;
if a;
if execut状态 = "FINISHED" and 复贷基本规则策略结果 = "" then 复贷基本规则策略结果 = "Accept";
...
if execut状态 = "FINISHED" and 复贷准入策略结果 = "" then 复贷准入策略结果 = "Accept";
