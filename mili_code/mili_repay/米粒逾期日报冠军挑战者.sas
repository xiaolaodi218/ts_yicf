...
data month1day;
set repayFin.Milipayment_report(keep=contract_no od_days cut_date 贷款余额 账户标签 客户标签 策略标签 loc_abmoduleflag PRODUCT_NAME)	;
...
if 策略标签="冠军" then 流转标签="冠军";
if 策略标签="挑战者" then 流转标签="挑战者";
if 客户标签 ^=1 then 流转标签="复贷客户";
...
else if 3<=last_oddays<=5 then 期初标签="02:4-6";
else if 6<=last_oddays<=14 then 期初标签="03:7-15";
else if 15<=last_oddays<=29 then 期初标签="04:16-30";
else if 30<=last_oddays<=59 then 期初标签="06:31-60";
else if 60<=last_oddays<=89 then 期初标签="07:61-90";

if 1<=od_days<=2 or (od_days in (0,.) and 还款_当日扣款失败合同=1)then 期末标签="01:1-3";
else if 3<=od_days<=5 then 期末标签="02:4-6";
else if 6<=od_days<=14 then 期末标签="03:7-15";
else if 15<=od_days<=29 then 期末标签="04:16-30";
else if 30<=od_days<=59 then 期末标签="06:31-60";
else if 60<=od_days<=89 then 期末标签="07:61-90";

*新增;
if ((od_days=0 and 还款_当日扣款失败合同=1) or 
((od_days=3 or od_days =6 or od_days=15  or od_days=30 or od_days=60 ) and last_oddays<od_days))
then  新增=1;else 新增=0;

*好转流出;
if ((1<=last_oddays<=2 or (last_oddays=0 and last_还款_当日扣款失败合同=1)) and od_days<1) or 
   (3<=last_oddays<=5 and od_days<3) or 
   (6<=last_oddays<=14 and od_days<6) or 
...
if ((1<=last_oddays<=2 or (last_oddays=0 and last_还款_当日扣款失败合同=1)) and od_days>2) or 
   (3<=last_oddays<=5 and od_days>5) or
   (6<=last_oddays<=14 and od_days>14) or
...
data st1 st2;
set cc2;
if 期末标签 in ("01:1-3","02:4-6","03:7-15","04:16-30") then output st1;
else output st2;
run;

x  "F:\米粒逾期日报表\米粒日逾期流转报表.xlsx"; 
filename DD DDE 'EXCEL|[米粒日逾期流转报表.xlsx]Sheet1!r4c5:r7c9';
data _null_;set st1;file DD;put 期初 新增  好转流出 恶化流出 催回;run;
filename DD DDE 'EXCEL|[米粒日逾期流转报表.xlsx]Sheet1!r4c11:r7c12';
data _null_;set st1;file DD;put 期末 贷款余额;run;

filename DD DDE 'EXCEL|[米粒日逾期流转报表.xlsx]Sheet1!r9c5:r10c9';
data _null_;set st2;file DD;put 期初 新增  好转流出 恶化流出 催回;run;
filename DD DDE 'EXCEL|[米粒日逾期流转报表.xlsx]Sheet1!r9c11:r10c12';
...
proc import datafile="F:\米粒逾期日报表\米粒日逾期流转报表配置表副本.xls"
...
TOTAL_TAT_b1=4+_n_*10;
TOTAL_TAT_b2=9+_n_*10;
TOTAL_TAT_e1=7+_n_*10;
TOTAL_TAT_e2=10+_n_*10;
