...
*-------------------------订单的标签--------------------------;
data apply_flag;
set submart.apply_submart(keep = apply_code 首次申请 订单类型);
run;
...
**拥有客户标签和AB标签的数据集;
data apply_flag;
merge apply_flag(in = a) silver_apply(in = b) abmoduleflag(in=c);
by apply_code;
if a;
run;
proc sort data = apply_flag nodupkey; by apply_code; run;
...
merge loan_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloan_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloanbqs_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloansimplebqs_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloantd_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge loanBQS_blk_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge loanBQS_loan_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge loanBQS_decision_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge loanBQS_face_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge Bqsrule_ycsq_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge loanBQS_invi_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge loanBQS_ivs_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloanBQS_loan_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloansimpleBQS_loan_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloanBQS_face_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloansimpleBQS_face_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge loanstrategyset_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloanstrategyset_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloansimplestrategyset_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge loangoldfunnel_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge loansilverfunnel_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge loanfunnel_submart(in = a) apply_flag(in = b);
...
merge reloanfunnel_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge reloansimplefunnel_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge BQSrule_jbgz_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge BQSrule_fsyys_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge BQSrule_fsds_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge BQSrule_glgz_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge BQSrule_mddh_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge CXrule_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge BQSrule_fdjbgz_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge BQSrule_fdzr_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
...
merge BQSrule_br_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge BQSrule_shixin_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge BQSrule_eysq_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge BQSrule_face_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge TDrule_submart(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
...
merge cx_anti_fraud(in = a) apply_flag(in = b);
by apply_code;
if a;
run;
